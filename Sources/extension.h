...
h = temp1 + temp2;                       \
    ...
    static void
    sha512step(sqlite3_context *ctx, int nargs, sqlite3_value **args)
    {
        ...
        // get the existing buffer/hash
        char* buffer = sqlite3_aggregate_context(ctx, SHA512_DIGEST_LENGTH);
        ...
        void* value = (void*)sqlite3_value_blob(args[0]);
        void* combined = malloc(SHA512_DIGEST_LENGTH + sizeof(value));
        memset(combined, 0, SHA512_DIGEST_LENGTH + sizeof(value));
        memcpy(combined, buffer, SHA512_DIGEST_LENGTH);
        memcpy(combined+SHA512_DIGEST_LENGTH, value, sizeof(value));
        
        // now hash it to see if we start within range
        uint8_t *hash = malloc(SHA512_DIGEST_LENGTH);
        memset(hash, 0, SHA512_DIGEST_LENGTH);
        SHA512(combined, sizeof(combined), hash);
    ...
    static void
    sha512finalize(sqlite3_context *ctx)
    {
        
        // get the existing buffer/hash
        char* buffer = sqlite3_aggregate_context(ctx, SHA512_DIGEST_LENGTH);
        sqlite3_result_blob(ctx, buffer, SHA512_DIGEST_LENGTH, NULL);
